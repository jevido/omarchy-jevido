...
// ---- Calendar feed helpers. The panel reads events.json, which is written by
//      omarchy-calendar-sync out of the ICS feeds in calendars.json. Keeping
//      the parsing here means the panel holds markup and nothing else.
...
function parsePayload(raw) {
  var empty = { calendars: [], days: {}, generatedAt: "" }
  ...
  try {
    ...
    // A half-written file is the one thing that must not blank the panel;
    // the sync writes atomically, so this only ever sees genuine corruption.
    return empty
  }
}

// A reminder wants a time as well as a day. Nine in the morning is the answer
// for any day that has not started yet; for today it would already be in the
// past, so round up to the next half hour instead.
function defaultReminderTime(key, now) {
  var target = dateFromKey(key)
  var today = new Date(now.getFullYear(), now.getMonth(), now.getDate())
  if (target > today) return "09:00"

  var minutes = now.getHours() * 60 + now.getMinutes()
  var next = Math.ceil((minutes + 5) / 30) * 30
  if (next >= 24 * 60) return "23:55"
...
// The feeds each report their own state; the panel shows one line per
// account so "no events" and "the feed is broken" never look the same.
function accountLine(account) {
  if (!account) return ""
  var status = String(account.status || "")
  if (status === "unconfigured") return "no feed URL yet"
  if (status === "error") return "failed: " + (account.error || "unknown error")
  if (status === "stale") return "last good copy — " + (account.error || "refresh failed")
  return ""
}

// The settings list names the address it is syncing and, only when there is
// one, the problem with it. A working feed's event total is a number nobody
// makes a decision with.
function accountSummary(account) {
  if (!account) return ""
  var name = String(account.email || account.name || "")
  var line = accountLine(account)
  return line === "" ? name : name + " · " + line
}

function accountOk(account) {
  return account && String(account.status || "") === "ok"
}
